/******************************************************************************
	...
	jquery.transformable.js v0.3
		...
		- added option 'sizeIcons', which will attempt to keep icons a fixed size within transformed element.
			...
			to disable: el.transformable('option','sizeIcons',false); tip
...
//console.log("originusesborder="+originusesborder);
	...
	function enablescale(tf) {
		...
		//moved to upper left, - forget double click thing
			tf.append('<div class="transformable-handle-scale transformable-handle ui-icon ui-icon-search" title="Zoom" style="left: 1px; top: 1px; position: absolute; z-index: 2000;"></div>');
...
*/
	...
	function enablerotate(tf) {
		tf.append('<div title="Rotar" class="transformable-handle-rotate transformable-handle ui-icon ui-icon-arrowrefresh-1-s" style="right: 1px; top: 1px; position: absolute; z-index: 2000;"></div>');
	...
	jQuery.fn.setTransform = function(options,args) {
	   return this.each( function () {
	    ...
	    if (opts.sizeIcons) fixiconsize(t); // ExClé FIX: Coin Size on setTransform
